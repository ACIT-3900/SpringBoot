...
import cstOptions.storage.StorageFileNotFoundException;
...
public class FileUploadController {
	...
	private boolean fileExtensionCheck(MultipartFile file) {
		...
		String acceptedType = "csv";
		
		String fileName = file.getOriginalFilename();
		String[] tokens = fileName.split("\\.(?=[^\\.]+$)");
		
		if (!tokens[1].toString().equals(acceptedType)){
			System.out.println(tokens[1] + " " + acceptedType);
			return true;
			} 
		return false;
	}
